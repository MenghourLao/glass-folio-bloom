...
export default function PricingSection() {
  ...
  return <section className="container mx-auto py-20 px-4 bg-inherit">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((plan, index) => <div key={index} className={`glass-card p-8 rounded-3xl flex flex-col h-full relative ${plan.popular ? 'border-primary' : ''}`}>
            {plan.popular && <div className="absolute top-0 right-6 -translate-y-1/2 bg-primary text-white px-4 py-1 rounded-full text-sm font-medium">
                Popular
              </div>}
            ...
            {plan.addons && plan.addons.length > 0 && <>
                ...
                <ul className="mb-6 space-y-3">
                  {plan.addons.map((addon, i) => <li key={i} className="flex items-start">
                      <div className="mr-3 mt-1">
                        <Check className="h-5 w-5 text-primary" />
                      </div>
                      <span>{addon}</span>
